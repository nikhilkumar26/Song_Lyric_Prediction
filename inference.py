...
classes = ['driver_license_front', 'atpi','utility_bill','phone_bill', 'bank_statement', 'paystub', 'other_id','voided_check', 'ach_withdrawal_acknowledgment_form']
...
#all_label = ['retail_installment_sales_contract', 'paystub', 'bank_statement', 'credit_approval', 'buyers_order', 'credit_application', 'vehicle_service_contract', 'buy_program', 'contacts', 'cover_sheet', 'odometer_disclosure_statement_retail', 'gap_binder', 'electronic_consent', 'driver_license_front', 'atpi', 'title_application', 'phone_bill', 'insurance_id_card', 'nada_bookout_sheet', 'utility_bill', 'risk_based_pricing_notice']

#all_label = ['driver_license_front', 'cover_sheet']


df = pd.read_csv("/home/ec2-user/doc_former_60k.csv")
...
df = df.groupby('type').apply(lambda x:x.tail(200))
#df = df.sample(50)
...
def main():
    ...
    docformer_load = docformer.load_from_checkpoint("models/epoch=6-step=2099.ckpt").to(device)
...
#from tqdm.auto import tqdm
    ...
    model = docformer_load.docformer.resnet.to(device)
    ...
    # move batch to device
        ...
        #print("BATCH", batch)
        with torch.no_grad():
            outputs = docformer_load(batch)
        
        #print("OUTPUTS", outputs[0].argmax(-1), batch['label'])
        predictions = outputs[0].argmax(-1)
        true_predictions = predictions.detach().cpu().clone().numpy().item()
        true_labels = batch['label'].detach().cpu().clone().numpy()
        #itrue_predictions, true_labels = get_labels(predictions, batch["labels"])
        print("True labels", true_labels)
        print("true_predictions", true_predictions)
        #eval_metric.add_batch(references=true_labels, predictions=true_predictions)
        true_pred.append(true_predictions)
        true_label.append(true_labels)

    
    from sklearn.metrics import accuracy_score, classification_report
    print("Accuracy", accuracy_score(true_label, true_pred))
    print("Classification Report", classification_report(true_label, true_pred))
    '''
