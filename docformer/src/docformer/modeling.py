...
class DocFormerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.x_topleft_position_embeddings_v = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.x_bottomright_position_embeddings_v = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.w_position_embeddings_v = nn.Embedding(config["max_2d_position_embeddings"]+1, config["shape_size"])
        ...
        self.y_topleft_position_embeddings_v = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.y_bottomright_position_embeddings_v = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.h_position_embeddings_v = nn.Embedding(config["max_2d_position_embeddings"]+1, config["shape_size"])
        ...
        self.x_topleft_position_embeddings_t = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.x_bottomright_position_embeddings_t = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.w_position_embeddings_t = nn.Embedding(config["max_2d_position_embeddings"]+1, config["shape_size"])
        ...
        self.y_topleft_position_embeddings_t = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.y_bottomright_position_embeddings_t = nn.Embedding(config["max_2d_position_embeddings"]+1, config["coordinate_size"])
        self.h_position_embeddings_t = nn.Embedding(config["max_2d_position_embeddings"]+1, config["shape_size"])
    ...
    def forward(self, x_feature, y_feature):
        ...
        #print("Clamping and adding a bias for handling negative values",batch," ", seq_len, " ",num_feat, " ", y_feature.shape[:-1])
        ...
        x_feature = torch.abs(x_feature)
        y_feature = torch.abs(y_feature)

        #print("Clamping ended")
        ...
        #print("positional")
        #print("TORCH", torch.max(x_feature), "MIN values", torch.min(x_feature))
        #print("TORCH", torch.max(y_feature),  "MIN values", torch.min(y_feature))
        #print("XFEATURE", x_feature[:,:,2], x_feature[:,:,2].shape)
...
class RelativePosition(nn.Module):
    ...
    def forward(self, length_q, length_k):
        #print("LENGTH", length_q, "LENGTH", length_k, "LENGTH SHAPE")
        embeddings = self.embeddings_table[self.final_mat[:length_q, :length_k]]
        #print("BROKE", embeddings)
        return embeddings
