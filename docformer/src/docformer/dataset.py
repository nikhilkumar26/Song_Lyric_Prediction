...
#     actual_bboxes = ocr_df.apply(get_topleft_bottomright_coordinates, axis=1).values.tolist()
   ...
   # assert len(words) == len(actual_bboxes)
    #print("IMAGE:",image_fp)
    words = ast.literal_eval(df[df['file_names']==image_fp]['text'].item())[:512]
    actual_bboxes = ast.literal_eval(df[df['file_names']==image_fp]['bounds'].item())[:512]
    print("words",words, "boxes", actual_bboxes)
    return {"words": words, "bbox": actual_bboxes}

def get_tokens_with_boxes(unnormalized_word_boxes, pad_token_box, word_ids,max_seq_len = 256):
...
def get_relative_distance(bboxes, centroids, pad_tokens_start_idx):
    ...
    print("len_a", len(a_rel_x))
    if len_a != 2048:
        a_rel_x.append(([0]*8)*(2048-len_a))
        a_rel_y.append(([0]*8)*(2048-len_b))       
    print("x_Feattures",len(a_rel_x))
    print("y_features", len(a_rel_y))
...
def create_features(
    ...
    #print("USE_OCR",use_ocr, "resized_image width", width, "resized_image_height",height,"boudnding_box",bounding_box)
    ...
    #print("BEFORE",resized_and_aligned_bboxes.shape)
    #if resized_and_aligned_bboxes.shape[0] < 1024:
    #    resized_and_aligned_bboxes.append([0]*4)
    

    print("resized_and_aligned shape", len(resized_and_aligned_bboxes))
    if len(resized_and_aligned_bboxes) < 2047:
        
        resized_and_aligned_bboxes.append([0] * 4)
        
        #print("PRINT", len(resized_and_aligned_bboxes))
    ...
    for k, v in encoding.items():
       # print("ENCODING:   ",k," " ,encoding[k].shape)
        encoding[k] = torch.as_tensor(encoding[k])
    
    #print("A_REL_X", len(a_rel_x), a_rel_x)
    encoding.update({
    ...
    #print("BEFORE",encoding["resized_and_aligned_bounding_boxes"].shape)
    #if encoding["resized_and_aligned_bounding_boxes"].shape[0] < 1024:
    #    encoding["resized_and_aligned_bounding_boxes"].append([0]*4)

    #print("BEFORE",resized_and_aligned_bboxes.shape)
    #if resized_and_aligned_bboxes.shape[0] < 1024:
    #    resized_and_aligned_bboxes.append([0]*4)

    #print("ENCODEING",encoding["resized_and_aligned_bounding_boxes"].shape,encoding["resized_and_aligned_bounding_boxes"].unsqueeze_(0).shape)
    if add_batch_dim:
    ...
    #if len(encoding['resized_scaled_img']) < 1024:
    #    print("Encoding", encoding['resized_scaled_img'].shape)    
    #    encoding["resized_scaled_img"].unsqueeze_(0)
    #    print("Encoding", encoding['resized_scaled_img'].shape)
    ...
    #for key, value in final_encoding.items():
        #if (key == 'resized_and_aligned_bounding_boxes'):
           # if value.shape[0] != 2048:
               # print("VALLL", value.shape)
